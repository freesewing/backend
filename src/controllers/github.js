...
const createPullRequest = async (repo, handle, username) => {
  return octokit.pulls.create({
    ...
    body:
      'This pull request rounds up changes made by ' +
      username +
      ' (freesewing user ' +
      handle +
      ') through the online editor on the website.\n\n' +
    ...
    octokit.repos
      ...
      .then(result => {
        ...
        if (!branch) {
          ...
          createBranch(req.body.repo, branchName, develop.commit.sha).then(result => {
            createOrUpdateFile(req.body.repo, branchName, req.body.file, data).then(result => {
              let commit = result.data.commit.html_url
              getPullRequests(req.body.repo, user.handle).then(result => {
                let pr
                if (result.data.length > 0) {
                  ...
                  createPullRequest(req.body.repo, user.handle, user.username).then(result => {
                    pr = {
